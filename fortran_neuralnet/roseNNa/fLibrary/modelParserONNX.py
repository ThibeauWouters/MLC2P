...
# TODO
# - specify output name instead of onnxModel.txt, but probably then have to change roseNNa workings as well?
...
# NEW -- just give name
filepath = "../goldenFiles/" + name + "/"
file    = filepath + name + ".onnx"
weigths = filepath + name + "_weights.onnx"
...
for node in nodes:
    print(node.op_type)
with open(filepath + 'onnxModel.txt','w') as f, open(filepath + 'onnxWeights.txt', 'w') as f2:
